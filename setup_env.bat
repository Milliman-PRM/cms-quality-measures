...
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Setting up nyhealth-cms-quality-measures env
...
echo.
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Redirecting NYHEALTH_CMS_QUALITY_MEASURES_HOME to local copy
SET NYHEALTH_CMS_QUALITY_MEASURES_HOME=%~dp0%
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: NYHEALTH_CMS_QUALITY_MEASURES is now %NYHEALTH_CMS_QUALITY_MEASURES_HOME%

echo.
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Prepending local copy of python library to PYTHONPATH
set PYTHONPATH=%NYHEALTH_CMS_QUALITY_MEASURES_HOME%python;%PYTHONPATH%
echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: PYTHONPATH is now %PYTHONPATH%

echo %~nx0 %DATE:~-4%-%DATE:~4,2%-%DATE:~7,2% %TIME%: Finished setting up nyhealth-cms-quality-measures env
